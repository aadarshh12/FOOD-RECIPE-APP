  ...
  return (
    <div className="App">
     ...
     <TextField id="outlined-basic" variant="outlined" type = "text" value={search} onChange={(e) => setSearch(e.target.value)} 
       />
     <Button variant="contained" color="secondary" style={{width: "80px", height: "50px"}} onClick={updateSearch}>
        Search
      </Button>
     </form>
     <Grid container className={gridClasses.root} container spacing={2}>
      <Grid item xs={12}>
      ...
      />
      <CardContent>
        <Typography variant="body2" color="text.secondary" components="p">
      ...
      <Collapse in={index === cardValue && expanded} timeout="auto" unmountOnExit>
        <CardContent>
          <Typography paragraph variant="h5">Material used:</Typography>
          {item.recipe.ingredients.map((item) => (
            <Typography paragraph>{item.text}
          </Typography>
          ))}
        </CardContent>
      </Collapse>
